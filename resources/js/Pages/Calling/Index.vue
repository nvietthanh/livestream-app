            ...
            <div class="border mt-2 p-[12px] grid grid-cols-5">
                ...
                >
                    <div class="flex items-center gap-2">
                        <div class="relative rounded-[50%] bg-[#dad7d7] border flex items-center justify-center w-[40px] h-[40px]">
                            <span class="text-[red] font-bold ">{{ item.name[0] }}</span>
                            <div v-if="onlineUsers.includes(item.id)" class="absolute bottom-[2px] right-[2px] w-[10px] h-[10px] rounded-[50%] bg-[green]" />
                        </div>
                        <span>{{ item.name }}</span>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        handShaking() {
            Echo.private(`handshake.${this.user.id}`)
                .listen("SendHandShake", (data) => {
        ...
        async setupCall(isOwner = false) {
            ...
            if (true) {
                ...
                this.localStream.getTracks().forEach((track) => {
                    this.senderTrack = this.peerConnection.addTrack(track, this.localStream);
                });
